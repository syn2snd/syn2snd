...
void audio_callback(void *data, Uint8 * stream, int len)
{
  ...
  if (!in) {
    ...
    fft_plan =
        fftw_plan_r2r_1d(have_samples, in, out, FFTW_HC2R, FFTW_BACKWARD);
  }
  ...
  for (i = 0; i < have_samples / 4; i++) {
    ...
    for (j = 0; j < step_size; j++) {
      int intensity =
          max(0, SOUND_MAX_INTENSITY - SOUND_DECAY_FACTOR *
                 (iteration - last_seen[i * step_size + j]));
      if (intensity)
        active_ports++;
  ...
  for (i = 0; i < len / sizeof(int32_t); i++) {
    int32_t_stream[i] =
        0.50 * INT_MAX * out[i] / ((active_ports + 1) *
                                   SOUND_MAX_INTENSITY);
  }
}
...
int main(int argc, char *argv[])
{
  ...
  window = SDL_CreateWindow
      ("syn2snd", SDL_WINDOWPOS_UNDEFINED,
       ...
       256 * PIXELS_PER_PORT, 256 * PIXELS_PER_PORT, SDL_WINDOW_SHOWN);
  ...
  for (iteration = 0;; iteration++) {
    ...
    if (iteration % REDRAW_PERIOD == 0) {
      ...
      for (i = 0; i < 65536; i++) {
        ...
        float intensity = (float) max(0,
                                      255 -
                                      COLOR_DECAY_FACTOR * (iteration -
                                                            last_seen
                                                            [i])) / 255.0;
        unsigned int addr = last_seen_addr[i];
        unsigned char r = (addr >> 16) & 0xff, g =
            (addr >> 8) & 0xff, b = addr & 0xff;
        ...
        if (last_seen_tcp[i]) {
          ...
          SDL_RenderDrawPoint(renderer,
                              rect.x + PIXELS_PER_PORT / 2,
                              rect.y + PIXELS_PER_PORT / 2);
        }
      }
    }
